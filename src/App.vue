<template>
  <div id="app">
    <header>
      ...
      <h2 class="headline">
        Generate a lore-friendly name for your Elder Scrolls character
      </h2>
    </header>
    <main class="content">
      <section>
    ...
    <footer class="footer">
      <p>
        Names sourced from
        <a href="https://uesp.net" target="blank">UESP</a> and
        <a href="https://tamriel-rebuilt.org" target="blank">Tamriel Rebuilt</a>
      </p>
    </footer>
  </div>
</template>
...
export default {
  ...
  methods: {
        ...
        switch (this.race) {
          ...
          default:
            name = this.getRandomName(
              this.gender == "male" ? list.male : list.female
            );
            break;
        }
    ...
    getOrcSurname(names) {
      ...
      return `${prefix}${this.getRandomName(
        this.gender == "male" ? names.male : names.female
      )}`;
    },
    ...
    getRedguardSurname(names) {
      ...
      const suffix = this.getRandomName(
        random === 2 ? names.birthplaces : names.male.concat(names.female)
      );
    ...
    getNordSurname(list) {
      if (Math.round(Math.random() * 1)) {
        ...
        return `${this.getRandomName(list.clanPrefix)}-${this.getRandomName(
          list.clanSuffix
        )}`;
      }
    },
    ...
    getReachmanSurname(list) {
      if (Math.round(Math.random() * 1)) {
        ...
        return `the ${this.getRandomName(
          list.titlePrefixes
        )} ${this.getRandomName(list.titleSuffixes)}`;
      }
    },
    ...
    getKhajiitName(list) {
      ...
      if (Math.random() * 1 <= chanceTitle) {
        ...
        if (isPrefix) {
          const prefix =
            this.gender === "male"
              ? this.getRandomName(titlesM).prefix
              : this.getRandomName(titlesF).prefix;
          name = `${prefix}'${name.toLowerCase()}`;
        } else {
          const suffix =
            this.gender === "male"
              ? this.getRandomName(titlesM).suffix
              : this.getRandomName(titlesF).suffix;
          name = `${name}-${suffix}`;
        }
      }
    ...
    getAshlanderName(list, isSurname) {
      const isHyphenated = Math.round(Math.random() * 1) ? true : false;
      const nameList = !isSurname
        ? this.gender === "male"
          ? list.male
          : list.female
        : list.surnames;
      let name = this.getRandomName(nameList);
      if (!isSurname ? isHyphenated : isHyphenated && name.length < 8)
        name += `-${this.getRandomName(nameList)}`;
